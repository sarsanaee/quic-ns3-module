...
void AddressTrackerLinux::Init() {
  netlink_fd_ = socket_ns3(AF_NETLINK, SOCK_RAW, NETLINK_ROUTE);
  if (netlink_fd_ < 0) {
  ...
  rv = HANDLE_EINTR(sendto_ns3(netlink_fd_, &request, request.header.nlmsg_len,
  ...
  rv = HANDLE_EINTR(sendto_ns3(netlink_fd_, &request, request.header.nlmsg_len, 0,
...
void AddressTrackerLinux::ReadMessages(bool* address_changed,
  ...
  *tunnel_changed = false;
  ...
  for (;;) {
    int rv = HANDLE_EINTR(recv_ns3(netlink_fd_,
                               buffer,
                               sizeof(buffer),
