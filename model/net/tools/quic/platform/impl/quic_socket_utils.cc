...
int QuicSocketUtils::SetGetAddressInfo(int fd, int address_family) {
  int get_local_ip = 1;
  int rc = setsockopt_ns3(fd, IPPROTO_IP, IP_PKTINFO, &get_local_ip,
                      sizeof(get_local_ip));
  if (rc == 0 && address_family == AF_INET6) {
    rc = setsockopt_ns3(fd, IPPROTO_IPV6, IPV6_RECVPKTINFO, &get_local_ip,
                    sizeof(get_local_ip));
  }
  return rc;
}
...
int QuicSocketUtils::SetGetSoftwareReceiveTimestamp(int fd) {
  ...
  return setsockopt_ns3(fd, SOL_SOCKET, SO_TIMESTAMPING, &timestamping,
                    sizeof(timestamping));
}

// static
bool QuicSocketUtils::SetSendBufferSize(int fd, size_t size) {
  if (setsockopt_ns3(fd, SOL_SOCKET, SO_SNDBUF, &size, sizeof(size)) != 0) {
...
bool QuicSocketUtils::SetReceiveBufferSize(int fd, size_t size) {
  if (setsockopt_ns3(fd, SOL_SOCKET, SO_RCVBUF, &size, sizeof(size)) != 0) {
...
int QuicSocketUtils::ReadPacket(int fd,
  ...
  int bytes_read = recvmsg_ns3(fd, &hdr, 0);
...
int QuicSocketUtils::CreateUDPSocket(const QuicSocketAddress& address,
  ...
  int fd = socket_ns3(address_family, SOCK_DGRAM | SOCK_NONBLOCK, IPPROTO_UDP);
  if (fd < 0) {
  ...
  int get_overflow = 1;
  int rc = setsockopt_ns3(fd, SOL_SOCKET, SO_RXQ_OVFL, &get_overflow,
                      sizeof(get_overflow));
  if (rc < 0) {
