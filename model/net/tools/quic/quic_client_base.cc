...
namespace net {
  ...
  bool QuicClientBase::FinishConnect() {
    if(!connected() && GetNumSentClientHellos() > QuicCryptoClientStream::kMaxClientHellos &&
        session() != nullptr && session()->error() == QUIC_CRYPTO_HANDSHAKE_STATELESS_REJECT) {
  ...
  bool QuicClientBase::ConnectLogic() {
