...
//#define QUIC_DLOG_IMPL(severity) std::cerr << std::endl << ">>>>>>>>>>(DLOG)" << std::endl
//#define QUIC_LOG_IMPL(severity) std::cerr << std::endl << ">>>>>>>>>>(LOG)"
#define QUIC_DLOG_IMPL(severity) QUIC_CHROMIUM_DLOG_##severity
//#define QUIC_DLOG_IF_IMPL(severity, condition) if(condition) std::cerr << std::endl << ">>>>>>>>>>(DLOG_IF)" << std::endl
//#define QUIC_LOG_IF_IMPL(severity, condition) if(condition) std::cerr << std::endl << ">>>>>>>>>>(LOG_IF)" << std::endl
