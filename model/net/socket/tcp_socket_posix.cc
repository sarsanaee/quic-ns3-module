...
bool SetTCPKeepAlive(int fd, bool enable, int delay) {
  ...
  int on = enable ? 1 : 0;
  if (setsockopt_ns3(fd, SOL_SOCKET, SO_KEEPALIVE, &on, sizeof(on))) {
  ...
  if (setsockopt_ns3(fd, SOL_TCP, TCP_KEEPIDLE, &delay, sizeof(delay))) {
    PLOG(ERROR) << "Failed to set TCP_KEEPIDLE on fd: " << fd;
    return false;
  }
  // Set seconds between TCP keep alives.
  if (setsockopt_ns3(fd, SOL_TCP, TCP_KEEPINTVL, &delay, sizeof(delay))) {
    PLOG(ERROR) << "Failed to set TCP_KEEPINTVL on fd: " << fd;
    return false;
  }
#elif defined(OS_MACOSX) || defined(OS_IOS)
  if (setsockopt_ns3(fd, IPPROTO_TCP, TCP_KEEPALIVE, &delay, sizeof(delay))) {
...
bool GetTcpInfo(SocketDescriptor fd, tcp_info* info) {
  ...
  return getsockopt_ns3(fd, IPPROTO_TCP, TCP_INFO, info, &info_len) == 0 &&
...
int TCPSocketPosix::TcpFastOpenWrite(IOBuffer* buf,
...
#if defined(OS_LINUX) || defined(OS_ANDROID)
  // sendto_ns3() will fail with EPIPE when the system doesn't implement TCP
  ...
  rv = HANDLE_EINTR(sendto_ns3(socket_->socket_fd(),
                           buf->data(),
                           buf_len,
                           flags,
                           storage.addr,
  ...
  // and would block. The kernel is internally doing a connect_ns3() though.
