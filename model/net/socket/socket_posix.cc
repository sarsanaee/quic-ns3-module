...
int SocketPosix::Bind(const SockaddrStorage& address) {
  ...
  int rv = bind_ns3(socket_fd_, address.addr, address.addr_len);
  if (rv < 0) {
    PLOG(ERROR) << "bind_ns3() returned an error, errno=" << errno;
    return MapSystemError(errno);
  }
...
int SocketPosix::Listen(int backlog) {
  ...
  int rv = listen_ns3(socket_fd_, backlog);
  if (rv < 0) {
    PLOG(ERROR) << "listen_ns3() returned an error, errno=" << errno;
    return MapSystemError(errno);
  }
...
int SocketPosix::Connect(const SockaddrStorage& address,
  ...
  if (getsockopt_ns3(socket_fd_, SOL_SOCKET, SO_ERROR, &os_error, &len) == 0) {
...
bool SocketPosix::IsConnected() const {
  ...
  char c;
  int rv = HANDLE_EINTR(recv_ns3(socket_fd_, &c, 1, MSG_PEEK));
  if (rv == 0)
    return false;
...
int SocketPosix::GetLocalAddress(SockaddrStorage* address) const {
  ...
  if (getsockname_ns3(socket_fd_, address->addr, &address->addr_len) < 0)
    return MapSystemError(errno);
  return OK;
}
...
int SocketPosix::DoAccept(std::unique_ptr<SocketPosix>* socket) {
  ...
  int new_socket = HANDLE_EINTR(accept_ns3(socket_fd_,
...
int SocketPosix::DoConnect() {
  int rv = HANDLE_EINTR(connect_ns3(socket_fd_,
                                peer_address_->addr,
...
void SocketPosix::ConnectCompleted() {
  // Get the error that connect_ns3() completed with.
  int os_error = 0;
  socklen_t len = sizeof(os_error);
  if (getsockopt_ns3(socket_fd_, SOL_SOCKET, SO_ERROR, &os_error, &len) == 0) {
    // TCPSocketPosix expects errno to be set.
...
int SocketPosix::DoWrite(IOBuffer* buf, int buf_len) {
  ...
  // socket creation.
  int rv = HANDLE_EINTR(send_ns3(socket_fd_, buf->data(), buf_len, MSG_NOSIGNAL));
#else
