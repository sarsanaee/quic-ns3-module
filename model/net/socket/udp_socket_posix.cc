...
int UDPSocketPosix::GetPeerAddress(IPEndPoint* address) const {
  ...
  if (!remote_address_.get()) {
    ...
    if (getpeername_ns3(socket_, storage.addr, &storage.addr_len))
      return MapSystemError(errno);
...
int UDPSocketPosix::GetLocalAddress(IPEndPoint* address) const {
  ...
  if (!local_address_.get()) {
    SockaddrStorage storage;
    if (getsockname_ns3(socket_, storage.addr, &storage.addr_len))
      return MapSystemError(errno);
    std::unique_ptr<IPEndPoint> address(new IPEndPoint());
    if (!address->FromSockAddr(storage.addr, storage.addr_len))
      return ERR_ADDRESS_INVALID;
...
int UDPSocketPosix::InternalConnect(const IPEndPoint& address) {
  ...
  // else connect_ns3() does the DatagramSocket::DEFAULT_BIND
  ...
  rv = HANDLE_EINTR(connect_ns3(socket_, storage.addr, storage.addr_len));
  if (rv < 0)
    return MapSystemError(errno);
...
int UDPSocketPosix::SetDoNotFragment() {
  ...
  if (addr_family_ == AF_INET6) {
    ...
    if (setsockopt_ns3(socket_, IPPROTO_IPV6, IPV6_MTU_DISCOVER, &val,
                   sizeof(val)) != 0) {
      return MapSystemError(errno);
    }
    ...
    if (getsockopt_ns3(socket_, IPPROTO_IPV6, IPV6_V6ONLY, &v6_only,
                   &v6_only_len) != 0) {
      return MapSystemError(errno);
    }
  ...
  int rv = setsockopt_ns3(socket_, IPPROTO_IP, IP_MTU_DISCOVER, &val, sizeof(val));
...
int UDPSocketPosix::SetBroadcast(bool broadcast) {
  ...
  // break.
  rv = setsockopt_ns3(socket_, SOL_SOCKET, SO_REUSEPORT, &value, sizeof(value));
  if (rv != 0)
    return MapSystemError(errno);
#endif  // defined(OS_MACOSX)
  rv = setsockopt_ns3(socket_, SOL_SOCKET, SO_BROADCAST, &value, sizeof(value));
...
int UDPSocketPosix::InternalRecvFrom(IOBuffer* buf,
  ...
  bytes_transferred =
      HANDLE_EINTR(recvfrom_ns3(socket_,
                            buf->data(),
                            buf_len,
                            flags,
                            storage.addr,
...
int UDPSocketPosix::InternalSendTo(IOBuffer* buf,
  ...
  int result = HANDLE_EINTR(sendto_ns3(socket_,
                            buf->data(),
                            buf_len,
                            0,
                            addr,
...
int UDPSocketPosix::SetMulticastOptions() {
  if (!(socket_options_ & SOCKET_OPTION_MULTICAST_LOOP)) {
    ...
    if (addr_family_ == AF_INET) {
      u_char loop = 0;
      rv = setsockopt_ns3(socket_, IPPROTO_IP, IP_MULTICAST_LOOP,
                      &loop, sizeof(loop));
    } else {
      u_int loop = 0;
      rv = setsockopt_ns3(socket_, IPPROTO_IPV6, IPV6_MULTICAST_LOOP,
                      &loop, sizeof(loop));
    }
    if (rv < 0)
      return MapSystemError(errno);
  ...
  if (multicast_time_to_live_ != IP_DEFAULT_MULTICAST_TTL) {
    ...
    if (addr_family_ == AF_INET) {
      ...
      rv = setsockopt_ns3(socket_, IPPROTO_IP, IP_MULTICAST_TTL,
                      &ttl, sizeof(ttl));
    } else {
      // Signed integer. -1 to use route default.
      int ttl = multicast_time_to_live_;
      rv = setsockopt_ns3(socket_, IPPROTO_IPV6, IPV6_MULTICAST_HOPS,
                      &ttl, sizeof(ttl));
    }
    if (rv < 0)
      return MapSystemError(errno);
    ...
    switch (addr_family_) {
      case AF_INET: {
        ...
        if (error != OK)
          return error;
#endif
        int rv = setsockopt_ns3(socket_, IPPROTO_IP, IP_MULTICAST_IF,
      ...
      case AF_INET6: {
        ...
        int rv = setsockopt_ns3(socket_, IPPROTO_IPV6, IPV6_MULTICAST_IF,
...
int UDPSocketPosix::DoBind(const IPEndPoint& address) {
  ...
  if (!address.ToSockAddr(storage.addr, &storage.addr_len))
    return ERR_ADDRESS_INVALID;
  int rv = bind_ns3(socket_, storage.addr, storage.addr_len);
  if (rv == 0)
    return OK;
  ...
  switch (group_address.size()) {
    case IPAddress::kIPv4AddressSize: {
      ...
      memcpy(&mreq.imr_multiaddr, group_address.bytes().data(),
             IPAddress::kIPv4AddressSize);
      int rv = setsockopt_ns3(socket_, IPPROTO_IP, IP_ADD_MEMBERSHIP,
                          &mreq, sizeof(mreq));
      if (rv < 0)
        return MapSystemError(errno);
      return OK;
    ...
    case IPAddress::kIPv6AddressSize: {
      ...
      memcpy(&mreq.ipv6mr_multiaddr, group_address.bytes().data(),
             IPAddress::kIPv6AddressSize);
      int rv = setsockopt_ns3(socket_, IPPROTO_IPV6, IPV6_JOIN_GROUP,
                          &mreq, sizeof(mreq));
      if (rv < 0)
        return MapSystemError(errno);
      return OK;
...
int UDPSocketPosix::LeaveGroup(const IPAddress& group_address) const {
  ...
  switch (group_address.size()) {
    case IPAddress::kIPv4AddressSize: {
      ...
      memcpy(&mreq.imr_multiaddr, group_address.bytes().data(),
             IPAddress::kIPv4AddressSize);
      int rv = setsockopt_ns3(socket_, IPPROTO_IP, IP_DROP_MEMBERSHIP,
                          &mreq, sizeof(mreq));
      if (rv < 0)
        return MapSystemError(errno);
      return OK;
    ...
    case IPAddress::kIPv6AddressSize: {
      ...
      memcpy(&mreq.ipv6mr_multiaddr, group_address.bytes().data(),
             IPAddress::kIPv6AddressSize);
      int rv = setsockopt_ns3(socket_, IPPROTO_IPV6, IPV6_LEAVE_GROUP,
                          &mreq, sizeof(mreq));
      if (rv < 0)
        return MapSystemError(errno);
      return OK;
...
int UDPSocketPosix::SetDiffServCodePoint(DiffServCodePoint dscp) {
  ...
  if (addr_family_ == AF_INET) {
    rv = setsockopt_ns3(socket_, IPPROTO_IP, IP_TOS,
                    &dscp_and_ecn, sizeof(dscp_and_ecn));
  } else {
    rv = setsockopt_ns3(socket_, IPPROTO_IPV6, IPV6_TCLASS,
                    &dscp_and_ecn, sizeof(dscp_and_ecn));
  }
  if (rv < 0)
    return MapSystemError(errno);
