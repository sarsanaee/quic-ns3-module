...
SocketDescriptor CreatePlatformSocket(int family, int type, int protocol) {
  ...
  if (result != kInvalidSocket && family == AF_INET6) {
    DWORD value = 0;
    if (setsockopt_ns3(result, IPPROTO_IPV6, IPV6_V6ONLY,
  ...
  return result;
#else  // OS_WIN
  SocketDescriptor result = socket_ns3(family, type, protocol);
#if defined(OS_MACOSX)
  ...
  if (result != kInvalidSocket) {
    int value = 1;
    if (setsockopt_ns3(result, SOL_SOCKET, SO_NOSIGPIPE, &value, sizeof(value))) {
      close(result);
      return kInvalidSocket;
    }
  }
