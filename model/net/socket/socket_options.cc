...
#endif
  int rv = setsockopt_ns3(fd, IPPROTO_TCP, TCP_NODELAY,
...
#endif
  int rv = setsockopt_ns3(fd, SOL_SOCKET, SO_REUSEADDR,
...
int SetSocketReceiveBufferSize(SocketDescriptor fd, int32_t size) {
  int rv = setsockopt_ns3(fd, SOL_SOCKET, SO_RCVBUF,
...
int SetSocketSendBufferSize(SocketDescriptor fd, int32_t size) {
  int rv = setsockopt_ns3(fd, SOL_SOCKET, SO_SNDBUF,
                      reinterpret_cast<const char*>(&size), sizeof(size));
#if defined(OS_POSIX)
  int os_error = errno;
#elif defined(OS_WIN)
